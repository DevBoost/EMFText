 ...
 */
public class Cct5Plugin extends Plugin {
	
	public static final String PLUGIN_ID = "org.emftext.test.cct5.resource.cct5";
	/**
	...
	private static Cct5Plugin plugin;
	...
	public static Cct5Plugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, Cct5Plugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, Cct5Plugin.PLUGIN_ID, message);
		}
		final Cct5Plugin pluginInstance = Cct5Plugin.getDefault();
