...
public class Cct5SyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public Cct5SyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public org.emftext.test.cct5.resource.cct5.mopp.Cct5LocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new org.emftext.test.cct5.resource.cct5.mopp.Cct5LocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public org.emftext.test.cct5.resource.cct5.mopp.Cct5LocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new org.emftext.test.cct5.resource.cct5.mopp.Cct5LocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new org.emftext.test.cct5.resource.cct5.mopp.Cct5LocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = org.emftext.test.cct5.resource.cct5.util.Cct5StringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
