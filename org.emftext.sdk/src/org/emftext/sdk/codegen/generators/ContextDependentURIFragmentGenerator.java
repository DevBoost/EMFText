...
public class ContextDependentURIFragmentGenerator extends BaseGenerator {
	...
	public boolean generate(PrintWriter out) {
		...
		sc.add("//");
		sc.add("public abstract class " + getResourceClassName() + "<ContainerType extends " + E_OBJECT + ", ReferenceType extends " + E_OBJECT + "> implements " + I_CONTEXT_DEPENDENT_URI_FRAGMENT + "<ReferenceType> {");
		sc.addLineBreak();
		...
		sc.add("protected " + I_REFERENCE_RESOLVE_RESULT + "<ReferenceType> result;");
		sc.addLineBreak();
		...
		sc.add("}");
		sc.addLineBreak();
		sc.add("public synchronized " + I_REFERENCE_RESOLVE_RESULT + "<ReferenceType> resolve() {");
		sc.add("if (resolving) {");
		sc.add("return null;");
		sc.add("}");
		...
		sc.addLineBreak();
		sc.add("" + I_REFERENCE_RESOLVER + "<ContainerType, ReferenceType> resolver = getResolver();");
		...
		sc.add("}");
		sc.addLineBreak();
		sc.add("public abstract " + I_REFERENCE_RESOLVER + "<ContainerType, ReferenceType> getResolver();");
		sc.addLineBreak();
		...
		sc.add("list = " + CAST_UTIL + ".cast(temp);");
		sc.add("}");
		sc.addLineBreak();
		sc.add("boolean first = true;");
		sc.add("for(" + I_REFERENCE_MAPPING + "<ReferenceType> mapping : result.getMappings()) {");
		sc.add("if (first) {");
		sc.add("first = false;");
		sc.add("} else if (list != null) {");
		sc.add("addResultToList(mapping, proxy, list);");
		sc.add("} else {");
		sc.add(EMFTEXT_RUNTIME_PLUGIN + ".logError(container.eClass().getName() + \".\" + reference.getName() + \" has multiplicity 1 but was resolved to multiple elements\", null);");
		sc.add("}");
		sc.add("}");
		sc.add("}");
		sc.addLineBreak();
		sc.add("private void addResultToList(" + I_REFERENCE_MAPPING + "<ReferenceType> mapping, " + E_OBJECT + " proxy, " + E_LIST + "<" + E_OBJECT + "> list) {");
		sc.add("" + E_OBJECT + " target = null;");
		sc.add("int proxyPosition = list.indexOf(proxy);");
		sc.addLineBreak();
		sc.add("if (mapping instanceof " + I_ELEMENT_MAPPING + "<?>) {");
		sc.add("target = ((" + I_ELEMENT_MAPPING + "<ReferenceType>) mapping).getTargetElement();");
		sc.add("} else if (mapping instanceof " + I_URI_MAPPING + "<?>) {");
		sc.add("target = " + ECORE_UTIL + ".copy(proxy);");
		sc.add(URI + " uri = ((" + I_URI_MAPPING + "<ReferenceType>) mapping).getTargetIdentifier();");
