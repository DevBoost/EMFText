...
public class EditorGenerator extends BaseGenerator {
	...
	public boolean generate(PrintWriter out) {
		...
		sc.add("resource = (" + I_TEXT_RESOURCE + ") resourceSet.getResource(uri, false);");
		sc.add("if (resource == null) {");
		sc.add("try {");
		sc.add(RESOURCE + " loadedResource = resourceSet.getResource(uri, true);");
		sc.add("if (loadedResource instanceof " + I_TEXT_RESOURCE + ") {");
		...
		sc.add("resource = (" + I_TEXT_RESOURCE + ") loadedResource;");
		...
		sc.add("}");
		sc.addLineBreak();
		sc.add("protected void setResource(" + I_TEXT_RESOURCE + " resource) {");
		...
		sc.add("//            has to be contained in the " + I_TEXT_RESOURCE + " of this editor.");
		sc.add("///");
		...
		sc.add("}");
		sc.add(I_SOURCE_VIEWER + " viewer = getSourceViewer();");
		sc.add(I_TEXT_RESOURCE + " textResource = (" + I_TEXT_RESOURCE + ") element.eResource();");
		...
		sc.addLineBreak();
		sc.add(I_TEXT_SCANNER + " lexer = resource.getMetaInformation().createLexer();");
		sc.add("try {");
		sc.add("lexer.setText(viewer.getDocument().get(destination, length));");
		sc.add(I_TEXT_TOKEN + " token = lexer.getNextToken();");
		...
		sc.add("}");
		sc.addLineBreak();
		sc.add("public void addBackgroundParsingListener(" + I_BACKGROUND_PARSING_LISTENER + " listener) {");
		sc.add("bgParsingListeners.add(listener);");
		sc.add("}");
		sc.addLineBreak();
		sc.add("public void notifyBackgroundParsingFinished() {");
		sc.add("for (" + I_BACKGROUND_PARSING_LISTENER + " listener : bgParsingListeners) {");
	...
	private void addMarkerAdapterClass(
		...
		sc.add("if (notifier != null && notifier instanceof " + I_TEXT_RESOURCE + ") {");
		sc.add(I_TEXT_RESOURCE + " resource = (" + I_TEXT_RESOURCE + ") notifier;");
	...
	private void addMarkerUpdateListenerClass(
			org.emftext.sdk.codegen.composites.StringComposite sc) {
		sc.add("private final class MarkerUpdateListener extends " + ABSTRACT_BACKGROUND_PARSING_LISTENER + " {");
	...
	private void addFields(org.emftext.sdk.codegen.composites.StringComposite sc) {
		...
		sc.add("private " + COLLECTION + "<" + I_BACKGROUND_PARSING_LISTENER + "> bgParsingListeners = new " + ARRAY_LIST + "<" + I_BACKGROUND_PARSING_LISTENER + ">();");
