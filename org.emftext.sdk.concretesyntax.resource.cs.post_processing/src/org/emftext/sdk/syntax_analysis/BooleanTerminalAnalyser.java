	...
	public void analyse(CsResource resource, ConcreteSyntax syntax) {
		...
		for (BooleanTerminal booleanTerminal : booleanTerminals) {
			...
			if (!(eFeature instanceof EAttribute)) {
				addProblem(resource, ECsProblemType.BOOLEAN_TERMINAL_FEATURE_NOT_ATTRIBUTE, FEATURE_NOT_ATTRIBUTE, booleanTerminal);
				continue;
			}
			EAttribute eAttribute = (EAttribute) eFeature;
			EClassifier eType = eAttribute.getEType();
			if (!eType.equals(EcorePackage.eINSTANCE.getEBoolean())) {
				addProblem(resource, ECsProblemType.BOOLEAN_TERMINAL_FEATURE_NOT_BOOLEAN, FEATURE_NOT_BOOLEAN, booleanTerminal);
				continue;
			}
			int upperBound = eAttribute.getUpperBound();
			if (upperBound != 1) {
				addProblem(resource, ECsProblemType.BOOLEAN_TERMINAL_WRONG_FEATURE_UPPER_BOUND, WRONG_UPPER_BOUND, booleanTerminal);
				continue;
			}
			String trueLiteral = booleanTerminal.getTrueLiteral();
			String falseLiteral = booleanTerminal.getFalseLiteral();
			if ("".equals(trueLiteral) && "".equals(falseLiteral)) {
				addProblem(resource, ECsProblemType.BOOLEAN_TERMINAL_BOTH_LITERALS_EMPTY, BOTH_LITERALS_EMPTY, booleanTerminal);
				continue;
			}
			if (trueLiteral.equals(falseLiteral)) {
				addProblem(resource, ECsProblemType.BOOLEAN_TERMINAL_EQUAL_LITERALS, EQUAL_LITERALS, booleanTerminal);
				continue;
			}
			if (genFeature == ConcreteSyntaxUtil.ANONYMOUS_GEN_FEATURE) {
				addProblem(resource, ECsProblemType.BOOLEAN_TERMINAL_ANONYMOUS_FEATURE, ANONYMOUS_FEATURE, booleanTerminal);
				continue;
			}
		}
	}
