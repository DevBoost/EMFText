SYNTAXDEF sumup
FOR <http://www.emftext.org/language/sumup>
START Sheet
...
TOKENS {
	DEFINE DECIMAL_FLOAT_LITERAL $ (('1'..'9') ('0'..'9')* | '0') '.' ('0'..'9')+ (('e'|'E') ('+'|'-')? ('0'..'9')*)?$;
...
TOKENSTYLES {
	...
	"IDENTIFIER" COLOR #000000, BOLD;
	"QUOTED_62_62" COLOR #FF0000;
}

RULES {
	Sheet ::= ("import" imports['"','"'])* units* constants* inputs* outputs* computations*;
	
	UnitDeclaration ::= "unit"#1 name[IDENTIFIER] (":" normalisation)? ".";

	InputVariable  ::= "input"  name[IDENTIFIER] ("in" #1 unit)? ".";
	OutputVariable ::= "output" name[IDENTIFIER] ("in" #1 unit)? ".";
	Constant       ::= "const"  name[IDENTIFIER] ("in" #1 unit)? ":" value ".";

	Assignment ::=
		("when" condition)?  
		"set" target[IDENTIFIER] #1 ":" #1 
		calculation #1 //( "=" #1 resultValue)? #1 
		".";
