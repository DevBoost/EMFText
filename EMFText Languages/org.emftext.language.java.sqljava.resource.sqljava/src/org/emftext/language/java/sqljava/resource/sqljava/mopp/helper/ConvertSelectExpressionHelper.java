...
//		}
	...
	public static List<Reference> convertColumnTerm(ColumnTerm columnTerm){
		...
		if(columnTerm.getTableReference() != null){
			...
			if(columnTerm.getTableReference().eIsProxy()){
							
				EObject resolvedObject = ResolverHelper.getResolvedProxie(
						columnTerm.getTableReference(),
						FromPackage.eINSTANCE.getTable());
				
				if (resolvedObject != null)
					references.add(ReferenceHelper.getNewStringReference(
							(String)resolvedObject.eGet(resolvedObject.eClass().getEStructuralFeature("name"))));
			}
			references.add(ReferenceHelper.getNewStringReference("."));
		}
		if(columnTerm.getColumn() != null){
			...
			Collection<LocalVariable> localVariables =
				JavaEObjectUtil.getObjectsByType(SqljavaBuilder.getResource().getAllContents(), VariablesPackage.eINSTANCE.getLocalVariable());
			boolean found = false;
