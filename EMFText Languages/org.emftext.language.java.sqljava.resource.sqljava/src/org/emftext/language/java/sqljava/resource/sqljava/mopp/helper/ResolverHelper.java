...
import org.emftext.language.java.sqljava.resource.sqljava.mopp.SqljavaBuilder;
...
public class ResolverHelper {
	...
	public static ConcreteClassifier getResolvedClassifier(String name, ClassifierReference container){
		
		ResolverHelper helper = new ResolverHelper();
		JavaReferenceResolveResult<Classifier> result =
			helper.new JavaReferenceResolveResult<Classifier>();
		
		ResolverHelper.resolve(name, container, null, -1, false, result);
		ConcreteClassifier concreteClassifier = null;
	...
	public static EObject getResolvedProxie(EObject proxie, EClass referencabledEClass){
		

		Collection<EObject> classInstances =
			SqljavaEObjectUtil.getObjectsByType(SqljavaBuilder.getResource().getAllContents(), referencabledEClass);
	
		for(EObject instance : classInstances){
			
			String instanceName = 
				(String)instance.eGet(instance.eClass().getEStructuralFeature("name"));
			
			URI proxieURI = 
				EcoreUtil.getURI(proxie);
			
			if(proxieURI.toString().endsWith(instanceName))
				return instance;
		}
