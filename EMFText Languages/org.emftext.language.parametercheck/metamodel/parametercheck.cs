...
TOKENS{
	...
	DEFINE CHARACTER_LITERAL $'\''('\\'('b'|'t'|'n'|'f'|'r'|'\"'|'\''|'\\')|('\\''u'('0'..'9'|'a'..'f'|'A'..'F')('0'..'9'|'a'..'f'|'A'..'F')('0'..'9'|'a'..'f'|'A'..'F')('0'..'9'|'a'..'f'|'A'..'F'))|('\\'('0'..'3')('0'..'7')('0'..'7')|'\\'('0'..'7')('0'..'7')|'\\'('0'..'7'))|~('\''|'\\'))'\''$ COLLECT IN literals; 
...
RULES{
	...
	ParameterCheckModel ::= method+ ;
	
	Method ::= "method" name[IDENTIFIER] "(" (parameter ("," parameter )*)? ")"  
					("check" check)+ ";" ;
	
	ParameterDecl ::= type[IDENTIFIER] name[IDENTIFIER];
	
	//Parameter ::= ignoreInMethodDecl[EXCLAM]? type[] name[];
	
	DeclaredParameter ::= (asString[NAME_TYPE] ":")? parameter[IDENTIFIER] ; 
	
	Literal ::= value[BOOLEAN_LITERAL] | value[CHARACTER_LITERAL] | value[STRING_LITERAL] 
				| value[HEX_FLOAT_LITERAL] | value[HEX_DOUBLE_LITERAL] | value[HEX_LONG_LITERAL] | value[HEX_INTEGER_LITERAL] 
				| value[DECIMAL_FLOAT_LITERAL] | value[DECIMAL_DOUBLE_LITERAL] | value[DECIMAL_LONG_LITERAL] | value[DECIMAL_INTEGER_LITERAL]
				| value[OCTAL_LONG_LITERAL] | value[OCTAL_INTEGER_LITERAL] ;

	Check ::= (exceptionIfTrue[EXCLAM])? name[IDENTIFIER]  
				"(" (parameterusage  ("," parameterusage)*)? ")" exception ;
	
	Exception::=  "throw" (package[IDENTIFIER] ":")? name[IDENTIFIER] "(" (parameterusage  ("," parameterusage)*)? ")" ;
