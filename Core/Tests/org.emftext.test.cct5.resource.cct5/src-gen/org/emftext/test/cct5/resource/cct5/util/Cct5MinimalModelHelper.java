 ...
 */
public class Cct5MinimalModelHelper {
	
	private final static org.emftext.test.cct5.resource.cct5.util.Cct5EClassUtil eClassUtil = new org.emftext.test.cct5.resource.cct5.util.Cct5EClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.emftext.test.cct5.resource.cct5.util.Cct5StringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.emftext.test.cct5.resource.cct5.util.Cct5ListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.emftext.test.cct5.resource.cct5.util.Cct5StringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = org.emftext.test.cct5.resource.cct5.util.Cct5ListUtil.castListUnchecked(value);
