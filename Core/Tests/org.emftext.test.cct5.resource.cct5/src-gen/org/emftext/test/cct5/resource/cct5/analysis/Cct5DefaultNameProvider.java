...
public class Cct5DefaultNameProvider implements org.emftext.test.cct5.resource.cct5.ICct5NameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.test.cct5.resource.cct5.util.Cct5EObjectUtil.invokeOperation(element, operation);
					if (result != null) {
